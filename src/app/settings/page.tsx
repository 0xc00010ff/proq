  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-2xl mx-auto py-10 px-6">
        <h1 className="text-2xl font-semibold text-bronze-900 dark:text-zinc-100 mb-1">Settings</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-8">Configure your proq instance.</p>

        <div className="space-y-8">
          {/* Appearance */}
          <section>
            <h2 className="text-sm font-medium text-bronze-800 dark:text-zinc-200 uppercase tracking-wider mb-4">Appearance</h2>
            <div className="space-y-4">
              <Field label="Theme">
                <select
                  value={settings.theme}
                  onChange={(e) => setSettings({ ...settings, theme: e.target.value as 'dark' | 'light' })}
                  className="w-full bg-bronze-100 dark:bg-zinc-900 border border-border-default rounded-md px-3 py-2 text-sm text-bronze-900 dark:text-zinc-100 focus:outline-none focus:border-steel"
                >
          ...
          {/* Agent */}
          <section>
            <h2 className="text-sm font-medium text-bronze-800 dark:text-zinc-200 uppercase tracking-wider mb-4">Agent</h2>
            <div className="space-y-4">
              <Field label="Claude binary path" hint="Path or command name for the Claude CLI. Defaults to &quot;claude&quot;.">
                <input
                  ...
                  placeholder="claude"
                  className="w-full bg-bronze-100 dark:bg-zinc-900 border border-border-default rounded-md px-3 py-2 text-sm font-mono text-bronze-900 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 focus:outline-none focus:border-steel"
                />
              </Field>
            </div>
          </section>
          ...
          {/* Notifications */}
          <section>
            <h2 className="text-sm font-medium text-bronze-800 dark:text-zinc-200 uppercase tracking-wider mb-4">Notifications</h2>
            <div className="space-y-4">
...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-sm font-medium text-bronze-700 dark:text-zinc-300 mb-1">{label}</label>
